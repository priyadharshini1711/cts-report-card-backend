...
const updateEventDetail = (event, id, request, response) => {
    ...
    update_query = "update event_detail set event_name = ?, objective = ?, description = ?, date = ?, place = ?, mode = ? where college_month_event_id = ?";
    connection.query(update_query, [event.event_name, event.objective, event.description, event.date, event.place, event.mode, event.college_month_event_id], (err, result) => {
        if (!err) {
            try {
                deleteEventStars(event.college_month_event_id, response, request)
                insertEventStars(event.event_star, response, request)
            } catch (e) {
                throw err
            }
        } else {
    ...
    checkIfEvenDetailExists(event.college_month_event_id, function (result) {
        ...
        if (college_month_event_id) {
            ...
            try {
                ...
                return response.status(200).json({ "message": "Insert Completed Successfully" })
            } catch (e) {
                console.log(e)
                return response.status(500).json(e)
            }
...
router.get('/event-star/', (request, response, next) => {
    ...
    where es.college_month_event_id = ${request.query.college_month_event_id} and es.status = 10`
